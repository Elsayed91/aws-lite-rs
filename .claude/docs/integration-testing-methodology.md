...
API docs lie. Discovery docs lie. Botocore models lie by omission. Only the real API tells the truth:

- AWS Config rejects `LIMIT` in SQL (must use request body `limit` field)
...
- **If the library does NOT support creating it** → use CLI (`aws`) via `std::process::Command`
...
**Fall back to CLI** (`aws`) via `std::process::Command` when the library doesn't support the operation needed for test setup. This is not a workaround — it's the correct approach for operations the library can't do yet.
...
fn aws_create_topic(name: &str, region: &str) -> Option<String> {
    let output = Command::new("aws")
        .args(["sns", "create-topic", "--name", name, "--region", region])
        .output()
...
fn aws_delete_topic(arn: &str, region: &str) {
    let _ = Command::new("aws")
        .args(["sns", "delete-topic", "--topic-arn", arn, "--region", region])
        .output();
}
```
...
- **Always pass explicit region** — never rely on defaults
...
assert!(format!("{err}").contains("InvalidExpressionException"));
...
- **Unsupported syntax** — keywords that look valid but aren't (e.g., SQL `LIMIT` in Config)
...
| Config discovery | Minutes to hours (use pre-existing resources or poll) |
...
├── cloudwatch.rs        # CloudWatch integration tests
├── config.rs            # Config integration tests
└── logs.rs              # Logs integration tests
```
